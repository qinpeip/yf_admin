...
// @ts-nocheck -- 若依原版预览算法，保持 JS 逻辑避免大规模改写
/* eslint-disable no-restricted-syntax -- 若依 Cron 预览算法使用带 label 的 for 循环 */
    ...
    goYear: for (let Yi = YIdx; Yi < YDate.length; Yi++) {
        ...
        goMonth: for (let Mi = MIdx; Mi < MDate.length; Mi++) {
            ...
            goDay: for (let Di = DIdx; Di < DDate.length; Di++) {
                ...
                // 如果日期规则中有值时
                switch (dayRule.value) {
                case 'assWeek': {
                    // 如果指定了是第几周的星期几
                    // 获取每月1号是属于星期几
                    let thisWeek = formatDate(new Date(YY + '-' + MM + '-' + DD + ' 00:00:00'), 'week');
                    DD = dayRuleSup.value[1] >= thisWeek ? (dayRuleSup.value[0] - 1) * 7 + dayRuleSup.value[1] - thisWeek + 1 : dayRuleSup.value[0] * 7 + dayRuleSup.value[1] - thisWeek + 1;
...
function getAssignArr(rule) {
    ...
    for (const [i, element] of assiginArr.entries()) {
